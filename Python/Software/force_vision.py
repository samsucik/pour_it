...
def force_on_robot(pattern_x_coord, pattern_size):
    ...
    if distance_from_center == 0 : distance_from_center += 0.0001
...
def drive(force):
    ...
    speed_leftM = 200
    if (force > 0):
        speed_leftM = base_speed - sigmoid(1/force)*100
        speed_rightM = base_speed + sigmoid(1/force)*100     
    else :
        speed_leftM = base_speed + sigmoid(1/force)*100
        speed_rightM = base_speed - sigmoid(1/force)*100
    #rightM.run_timed(time_sp=time_, speed_sp=speed_rightM)
    #leftM.run_timed(time_sp=time_, speed_sp=speed_leftM)
...
drive(force_on_robot(float(sys.argv[1]),float(sys.argv[2])))
